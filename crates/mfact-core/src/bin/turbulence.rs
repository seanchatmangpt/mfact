...
/// Simulates a workload representing a single node in a workflow DAG.
/// To make it physically real, it performs BLAKE3 hashing to match the `ggen` receipt engine overhead.
fn simulate_workload(iterations: usize) {
    let mut hash = blake3::hash(b"initial_seed");
...
fn main() {
    ...
    let scales = vec![100_000, 50_000, 10_000, 5_000, 1_000, 500, 100, 50, 10, 5, 1];
    
    let mut previous_density: Option<f64> = None;
    let mut previous_scale: Option<usize> = None;

    println!("{:<15} | {:<20} | {:<20}", "Work/Task", "Density (tasks/s)", "Scaling Exponent (α)");
    println!("{:-<60}", "");
    ...
    for &work_per_task in &scales {
        ...
        if let (Some(prev_dens), Some(prev_w)) = (previous_density, previous_scale) {
            ...
            if alpha > -0.5 && !phase_transition_detected {
                println!("{:-<60}", "");
                println!(">>> TURBULENT REGIME DETECTED (Phase Change) at Work/Task: {}", work_per_task);
                println!(">>> The execution density scaling exponent collapsed to {}.", exponent_str);
                println!(">>> This is the exact physical threshold parameter for the Sparse Bottleneck Router.");
                println!("{:-<60}", "");
                phase_transition_detected = true;
            }
        }
        
        println!("{:<15} | {:<20.2} | {:<20}", work_per_task, density, exponent_str);
