...
\paragraph{Code-to-proof extraction.} The correspondence pilot of
...
iff liveness and boundedness of its short-circuited net. The original
statement over arbitrary (possibly infinite) transition sets is false; a
countermodel exists where infinitely many transition pairs route unbounded
tokens through a net without deadlock, violating the original characterization.
We repaired this by adding a finiteness constraint on the transition set
(\(\text{Finite } T\)). The admitted theorem in \textsf{procint} is the
...
Beyond \textsf{procint}'s own catalog, \textsf{mfact} pilots a second
correspondence lane: obligations that relate a Lean declaration to code
extracted from an \emph{external}, unverified Rust crate
(\texttt{wasm4pm-core}) via Aeneas~\cite{ho2022aeneas} and its Charon
front end~\cite{protzenko2024charon}. This is deliberately scoped to one
obligation --- token-replay counts correspondence, ``D1'' --- and follows
the same status ladder as the rest of this paper's standing (declared,
extracted, stated, proven), computed by a dedicated builder
(\texttt{scripts/build\_verif.py}) from observed evidence: a charon/aeneas
pipeline receipt, a \texttt{lake build} of the extracted module against
\textsf{procint}, and an axiom-print check on the resulting theorem. The
