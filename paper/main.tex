...
State conformance and safety properties are structurally modeled via W3C SHACL (Shapes Constraint Language) and ShEx (Shape Expressions) constraint schemas. While the engine exposes offline validation via the \texttt{validate\_shacl} and \texttt{validate\_shex} functions, operating these as executable transaction-path admission gates remains a future design objective rather than a currently implemented feature. Safety violations and negative conditions are captured as N3 logical denials of the form \texttt{\{ body \} => false.}, where any deduction of falsity indicates a constraint violation.
...
Fast evaluation is a critical design requirement for these validations, with the future goal of integrating them directly on the transaction-path execution loop of the process engine. High latency during validation would directly degrade transaction throughput in such an operational deployment. To evaluate readiness for this future objective, we measure and record current performance characteristics empirically in Section~\ref{sec:rslab_benchmark} to verify that the baseline overhead of validation remains within acceptable limits.
