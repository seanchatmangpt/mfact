...
The draft contains 8 sections ready for main.tex integration:
...
**Status**: Hand-authored; ready for copy-paste to main.tex  
...
3. Step 8 (paper-render) includes `\input{correspondence_status.tex}` in main.tex
...
**Process**: Merge sections into main.tex; substitute auto-rendered components; lint prose  
**Output**: main.tex with correspondence sections + correspondence_status.tex included

**Integration Checklist**:
1. Run `just prose-lint` to verify hand-authored sections (exit 0 required)
2. Include `\input{target/mfact/paper/correspondence_status.tex}` in main.tex after §4
3. Copy Related Work (§1) and Crown Claim (§2) into main.tex Related Work section
4. Copy Novelty + Falsifiers (§3) into main.tex Background/Novelty section
5. Copy Measurable Claims (§4) as standalone section before correspondence_status.tex
6. Commit main.tex with all four sections integrated

**Exit Criteria**:
- All prose-lint checks pass (8 rules, exit 0)
- LaTeX compilation succeeds (`latexmk -pdf main.tex` exit 0)
- PDF output contains all four sections + auto-rendered correspondence_status table

### Step 9: Paper Finalization (post-wiring)

**Input**: Compiled main.pdf  
...
Insert §1 text into the Related Work section of main.tex, after existing related work and before Contribution section.
...
| main.tex | `/Users/sac/mfact/paper/main.tex` | ~36K | LaTeX | Integrate sections |
...
### Pitfall 1: TBD placeholders in main.tex before Step 7 runs

**Problem**: If you copy Measurable Claims (§4) into main.tex before Step 7, all values are "TBD". The PDF renders but is incomplete.

**Solution**: Do not integrate §4 into main.tex until Step 7 (build_verif.py) has populated the TBD slots. Either:
- Leave §4 in PAPER_SECTIONS_DRAFT.md as a staging area; merge only after Step 7 populates values.
- Or, include §4 in main.tex but wrap it in a LaTeX comment: `% \input{measurable_claims.tex}` until Step 7 is done.
...
**Problem**: correspondence_status.tex is at `target/mfact/paper/correspondence_status.tex`, but main.tex is at `target/mfact/paper/main.tex`. Relative path `\input{correspondence_status.tex}` works, but if main.tex is moved, the path breaks.

**Solution**: Use absolute path or consistent relative path. In main.tex: `\input{./correspondence_status.tex}` (same directory).
...
1. Merge PAPER_SECTIONS_DRAFT.md §1-4 into main.tex (following integration checklist).
2. Run `just prose-lint` to verify no prose violations.
3. Run `latexmk -pdf main.tex` to compile and verify LaTeX output.
4. Commit main.tex with correspondence sections integrated.
