...
**Grep Command**:
```bash
grep -n "Aeneas\s\+\(proves\|verified\|checked\|certified\)" main.tex PAPER_SECTIONS_DRAFT.md
```
...
**Grep Command**:
```bash
grep -nE "([^.]*)\b(verified|proven)\b([^.]*?)(\s|\.)" main.tex PAPER_SECTIONS_DRAFT.md | \
...
**Grep Command**:
```bash
grep -nE "^[^.]*\." main.tex PAPER_SECTIONS_DRAFT.md | \
...
**Grep Command**:
```bash
grep -n "automatically\|without proof\|without verif\|without check" main.tex PAPER_SECTIONS_DRAFT.md | head -20
```
...
**Grep Command**:
```bash
grep -nE "\s(chain|hash)\s" main.tex PAPER_SECTIONS_DRAFT.md | \
...
awk '/^(Claim|We claim|Our claim|We show|We prove|our approach)/ \
     { getline; getline; if (!/if\s|fail|reject|exit|error|contradict/) \
       print NR": " $0 }' main.tex PAPER_SECTIONS_DRAFT.md | head -20
```
...
**Grep Command**:
```bash
grep -nE "prove|proof|proven" main.tex PAPER_SECTIONS_DRAFT.md | \
...
**Grep Command**:
```bash
grep -nE "entirely|completely|fully|absolutely|always|never" main.tex PAPER_SECTIONS_DRAFT.md | \
...
prose-lint:
    ...
    # Rule 1: Forbid "Aeneas proves"
    @if grep -n "Aeneas\s\+\(proves\|verified\|checked\|certified\)" target/mfact/paper/main.tex target/mfact/paper/PAPER_SECTIONS_DRAFT.md 2>/dev/null; then \
        echo "ERROR: Rule 1 violation - 'Aeneas proves' detected (use 'Aeneas extracts' or 'Lean proves')."; \
        exit 1; \
    fi
    
    # Rule 2: Forbid unscoped "verified"
    @if grep -nE "([^.]*)\b(verified|proven)\b([^.]*?)(\s|\.)" target/mfact/paper/main.tex 2>/dev/null | \
    ...
    @if grep -n "automatically.*\(extract\|verif\|prove\|check\|generate\)\|without.*\(proof\|verif\|check\)" \
        target/mfact/paper/main.tex target/mfact/paper/PAPER_SECTIONS_DRAFT.md 2>/dev/null; then \
        echo "ERROR: Rule 4 violation - 'automatically' or 'without proof' detected (be specific)."; \
        exit 1; \
    fi
    
    # Rule 5: Require "receipt chain" specificity
    @if grep -E "\s(chain|hash)\s" target/mfact/paper/main.tex 2>/dev/null | \
    ...
    @if grep -E "\b(prove|proof|proven)\b" target/mfact/paper/main.tex 2>/dev/null | \
    ...
    @if grep -E "entirely|completely|fully|absolutely|always|never" \
        target/mfact/paper/main.tex target/mfact/paper/PAPER_SECTIONS_DRAFT.md 2>/dev/null | \
...
grep -nE "([^.]*)\b(verified|proven)\b([^.]*?)(\s|\.)" \
  target/mfact/paper/main.tex target/mfact/paper/PAPER_SECTIONS_DRAFT.md | \
  grep -v "via\|through\|using\|by\|when\|because"
