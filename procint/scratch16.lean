...
opaque TTLGraph : Type := Unit
opaque TeraTemplate : Type := Unit
opaque RustExecutable : Type := Unit

instance : Inhabited TTLGraph := ⟨()⟩
instance : Inhabited TeraTemplate := ⟨()⟩
instance : Inhabited RustExecutable := ⟨()⟩
