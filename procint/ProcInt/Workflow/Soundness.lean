...
by a dedicated lane. Requires `[Finite T]`: without finitely many transitions
there is a countermodel where `W.Sound` holds but the short-circuited net is
unbounded (tokens accumulate through an infinite family of transition pairs). -/
def WfNet.sound_iff_shortCircuit_live_bounded_statement {P T : Type} [DecidableEq P] [Finite T]
    (W : WfNet P T) : Prop :=
