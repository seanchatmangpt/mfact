...
-- names a theorem whose body is a bare `by exact trivial` — the search script splices
-- candidate tactic sequences into a scratch copy of this file in place of
-- that `by exact trivial` and checks the result against the Lean kernel. These are
