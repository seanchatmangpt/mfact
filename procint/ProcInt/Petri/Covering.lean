...
/-! # ProcInt.Petri.Covering

Coverability infrastructure for unbounded nets: repetition-free ('loop-cut') firing sequences, the prefix order on repetition-free runs (`PetriNet.Run`) with its well-founded strict order and `IsStronglyAtomic` instance, the Kőnig's-lemma application that extracts an infinite injective marking sequence from any unbounded net, and the resulting self-covering pump (`unbounded_self_cover`) obtained by combining that sequence with support confinement and Dickson's-lemma-style domination over finitely-supported markings (Karp & Miller 1969, Proc. 10th IEEE Symp. on Switching and Automata Theory; van der Aalst 1997, Verification of Workflow Nets, the boundedness direction of Lemma 8). -/
...
/-- `PetriNet.Run N M0`: repetition-free valid firing sequences from `M0`. -/
def PetriNet.Run {P T : Type} (N : PetriNet P T) (M0 : Marking P) : Type :=
  {σ : List T // ∃ M, N.FiringSeq M0 σ M ∧ (N.markingTrace M0 σ).Pairwise (· ≠ ·)}

/-- Every list-prefix of a valid run's transition list is itself a valid
run: `FiringSeq` restricts via `FiringSeq.split`, and trace
pairwise-distinctness restricts via `List.Pairwise.sublist` (a prefix is a
sublist). -/
theorem PetriNet.Run.isRun_of_prefix {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {σ σ' : List T}
...
/-- The prefix order on repetition-free runs. -/
instance PetriNet.Run.instPartialOrder {P T : Type} {N : PetriNet P T} {M0 : Marking P} :
    PartialOrder (PetriNet.Run N M0) where
  le r₁ r₂ := r₁.1 <+: r₂.1
...
theorem PetriNet.Run.le_iff {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r₁ r₂ : PetriNet.Run N M0) : r₁ ≤ r₂ ↔ r₁.1 <+: r₂.1 := Iff.rfl

def PetriNet.Run.bot {P T : Type} {N : PetriNet P T} {M0 : Marking P} : PetriNet.Run N M0 :=
  ⟨[], M0, PetriNet.FiringSeq.nil M0, by simp [PetriNet.markingTrace]⟩

instance PetriNet.Run.instOrderBot {P T : Type} {N : PetriNet P T} {M0 : Marking P} :
    OrderBot (PetriNet.Run N M0) where
  bot := PetriNet.Run.bot
  bot_le _ := List.nil_prefix

@[simp] theorem PetriNet.Run.bot_val {P T : Type} {N : PetriNet P T} {M0 : Marking P} :
    (⊥ : PetriNet.Run N M0).1 = [] := rfl

theorem PetriNet.Run.length_lt_of_lt {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {r₁ r₂ : PetriNet.Run N M0} (h : r₁ < r₂) :
...
/-- The strict prefix order on runs is well-founded: it embeds
monotonically into `(ℕ, <)` via transition-list length. -/
theorem PetriNet.Run.lt_wf {P T : Type} {N : PetriNet P T} {M0 : Marking P} :
    WellFounded (@LT.lt (PetriNet.Run N M0) _) :=
  have H : Subrelation (@LT.lt (PetriNet.Run N M0) _)
      (InvImage (· < ·) (fun r : PetriNet.Run N M0 => r.1.length)) :=
    fun {_ _} h => PetriNet.Run.length_lt_of_lt h
  Subrelation.wf H <| InvImage.wf _ wellFounded_lt

instance PetriNet.Run.instWellFoundedLT {P T : Type} {N : PetriNet P T} {M0 : Marking P} :
    WellFoundedLT (PetriNet.Run N M0) := ⟨PetriNet.Run.lt_wf⟩

/-- Repetition-free runs form a strongly atomic order (every strict
extension can be reached one atomic step at a time), via well-foundedness
of the strict prefix order. -/
instance PetriNet.Run.instIsStronglyAtomic {P T : Type} {N : PetriNet P T} {M0 : Marking P} :
    IsStronglyAtomic (PetriNet.Run N M0) :=
  IsStronglyAtomic.of_wellFounded_lt PetriNet.Run.lt_wf

def PetriNet.Run.takeRun {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r' : PetriNet.Run N M0) (i : ℕ) : PetriNet.Run N M0 :=
  ⟨r'.1.take i, PetriNet.Run.isRun_of_prefix r'.2 (List.take_prefix _ _)⟩

@[simp] theorem PetriNet.Run.takeRun_val {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r' : PetriNet.Run N M0) (i : ℕ) :
    (PetriNet.Run.takeRun r' i).1 = r'.1.take i := rfl

theorem PetriNet.Run.lt_take_succ_lt {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {r r' : PetriNet.Run N M0} (h : r < r') {i : ℕ}
    (hi1 : r.1.length ≤ i) (hi2 : i + 1 < r'.1.length) :
    r < PetriNet.Run.takeRun r' (i + 1) ∧ PetriNet.Run.takeRun r' (i + 1) < r' := by
  ...
  · refine ⟨hpre1, fun hcontra => ?_⟩
    ...
    simp only [PetriNet.Run.takeRun_val, List.length_take] at hlen2
    omega
...
theorem PetriNet.Run.covBy_iff_length_succ {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {r r' : PetriNet.Run N M0} (h : r < r') :
    r ⋖ r' ↔ r'.1.length = r.1.length + 1 := by
  constructor
  · intro hcov
    by_contra hne
    have hlt := PetriNet.Run.length_lt_of_lt h
    have hgap : r.1.length + 1 < r'.1.length := by omega
    obtain ⟨h1, h2⟩ := PetriNet.Run.lt_take_succ_lt h (le_refl r.1.length) hgap
    exact hcov.2 h1 h2
  · intro hlen
    refine ⟨h, fun c h1 h2 => ?_⟩
    have hc1 := PetriNet.Run.length_lt_of_lt h1
    have hc2 := PetriNet.Run.length_lt_of_lt h2
    omega

noncomputable def PetriNet.Run.coverTransition {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {r r' : PetriNet.Run N M0} (h : r ⋖ r') : T :=
  r'.1[r.1.length]'(by
    have hlen := (PetriNet.Run.covBy_iff_length_succ h.lt).mp h
    omega)

theorem PetriNet.Run.coverTransition_spec {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {r r' : PetriNet.Run N M0} (h : r ⋖ r') :
    r'.1 = r.1 ++ [PetriNet.Run.coverTransition h] := by
  have hlen := (PetriNet.Run.covBy_iff_length_succ h.lt).mp h
  ...
  · intro n h1 h2
    ...
    · have hn' : n = r.1.length := by omega
      subst hn'
      simp [PetriNet.Run.coverTransition, List.getElem_append_right]

/-- Given `[Fintype T]`, every run has only finitely many one-step
extensions (covers) — exactly the finiteness hypothesis required to apply
Mathlib's König-lemma-style `exists_orderEmbedding_covby_of_forall_covby_finite_of_bot`. -/
theorem PetriNet.Run.finite_setOf_covBy {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    [Fintype T] (r : PetriNet.Run N M0) :
    {x | r ⋖ x}.Finite := by
  have hinj : Function.Injective (fun x : {x // r ⋖ x} => PetriNet.Run.coverTransition x.2) := by
    rintro ⟨r₁, h₁⟩ ⟨r₂, h₂⟩ heq
    simp only at heq
    have hσ : r₁.1 = r₂.1 := by
      rw [PetriNet.Run.coverTransition_spec h₁, PetriNet.Run.coverTransition_spec h₂, heq]
...
noncomputable def PetriNet.Run.endMarking {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) : Marking P := r.2.choose

theorem PetriNet.Run.endMarking_spec {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) : N.FiringSeq M0 r.1 (PetriNet.Run.endMarking r) :=
  r.2.choose_spec.1

theorem PetriNet.Run.endMarking_pairwise {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) :
...
theorem PetriNet.Run.endMarking_eq_of_firingSeq {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    {r : PetriNet.Run N M0} {M : Marking P}
    (h : N.FiringSeq M0 r.1 M) : PetriNet.Run.endMarking r = M :=
  PetriNet.FiringSeq.deterministic N (PetriNet.Run.endMarking_spec r) h

/-- `endMarking r` is exactly the last entry of `r`'s marking trace. -/
theorem PetriNet.Run.endMarking_eq_get {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) :
    PetriNet.Run.endMarking r = (N.markingTrace M0 r.1).get
      ⟨r.1.length, by simp [PetriNet.markingTrace_length]⟩ := by
  have hsplit := (PetriNet.Run.endMarking_spec r).split N r.1.length le_rfl
...
theorem PetriNet.Run.reaches_endMarking {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) : N.Reaches M0 (PetriNet.Run.endMarking r) :=
  N.firingSeq_reaches (PetriNet.Run.endMarking_spec r)

/-- `endMarking_eq_get`, restated in `getElem` bracket notation. -/
theorem PetriNet.Run.endMarking_eq_getElem {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) :
    PetriNet.Run.endMarking r = (N.markingTrace M0 r.1)[r.1.length]'
      (by simp [PetriNet.markingTrace_length]) := by
  simpa [List.get_eq_getElem] using PetriNet.Run.endMarking_eq_get r

/-- Generalized form of `endMarking_eq_getElem`, letting the index be
supplied as an arbitrary `n` provably equal to `r.1.length`. -/
theorem PetriNet.Run.endMarking_eq_getElem' {P T : Type} {N : PetriNet P T} {M0 : Marking P}
    (r : PetriNet.Run N M0) (n : ℕ) (hn : n = r.1.length)
    (h : n < (N.markingTrace M0 r.1).length) :
    PetriNet.Run.endMarking r = (N.markingTrace M0 r.1)[n]'h := by
  subst hn; exact PetriNet.Run.endMarking_eq_getElem r
...
/-- An unbounded net has infinitely many runs from `M0`. -/
...
/-- **Kőnig's-lemma application.** An unbounded net admits an infinite,
injective sequence of markings, each reached from the previous by a single
`Step`. -/
...
/-- **Self-covering pump.** An unbounded net admits a marking `M` reachable
from `M0`, from which a nonzero surplus `R` can be re-added and remain
reachable — the Karp–Miller "self-covers" witness. -/
