...
theorem observableSpan_subset_invariant_test {Th : PlanningTheory} {α : Type}
  ...
  | cons c cs ih =>
    ...
    | cons φ bs =>
      ...
      dsimp [List.sum]
      -- List.sum is foldl usually. But in mathlib there might be `List.sum_cons`.
      -- Let's just use `simp [List.sum_cons]`. No, `List.sum` on `List ℝ` might be additive.
      -- Or just `simp`!
      -- Let's just use congr_arg.
      -- Wait, if it's just zipWith and sum, maybe `simp` can do it.
