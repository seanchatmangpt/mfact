...
theorem observableSpan_subset_invariant_test {Th : PlanningTheory} {α : Type}
  ...
  | cons c cs ih =>
    ...
    | cons φ bs =>
      ...
      -- Let's see what happens if we just use exact!
      -- We need to prove `c * φ.observe b₂ + sum_cs1 = c * φ.observe b₂ + sum_cs2`
      -- But wait, rw might not be doing what I think it is. Let's use `congr 1`.
      congr 1
      exact h_ih
