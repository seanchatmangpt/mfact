...
/--
  The Empirical Falsification Loop (Part XXXVIII of Notation Authority).
  It asserts that if a revocation condition is met for transformation `T` 
  and evidence `E` in context `ctx`, then the new context after revocation
  must have the status of `T` set to `Revoked`.
-/
